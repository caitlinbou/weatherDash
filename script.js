...
function displayWeatherInfo(){
    ...
    // variables to hold information for TODAY
      ...
      var icon = $("<img>").attr("src", `http://openweathermap.org/img/wn/${responseNow.weather[0].icon}@2x.png`);
    ...
    // for loop to render information for each day in five day forecast, with values assigned to variables stated prior
      ...
      for (var i = 1; i < 6; i++) {
        ...
        iconForecast = $("<img>").attr("src", `http://openweathermap.org/img/wn/${forecast[i].weather[0].icon}@2x.png`)
        maxTemp = $("<p class='card-text'>").text(`Temp high: ${Math.round(forecast[i].temp.max)}°F`)
        minTemp = $("<p class='card-text'>").text(`Temp low: ${Math.round(forecast[i].temp.min)}°F`)
        humidityI = $("<p class='card-text'>").text(`humidity: ${forecast[i].humidity}%`)
...
function renderBtns(){
    for (i = 0; i < cities.length; i++){
        ...
        localStorage.setItem(("city"), (cities), JSON.stringify(city));
    }
}      
// JSON.parse(localStorage.getItem(cities, "data-number"))
// Adding a click event listener to "citySearchBtn"
$(".citySearchBtn").on("click", function (event) {
    event.preventDefault()
// FIXME: how do I empty the iput field? If figured out, then do it on all click events. 
...
// TODO: if time, then set a limit of 10 cities by limiting length of cities array. (I know how to do this I think)
// TODO: if 
